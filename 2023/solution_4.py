...
def parse_game_data(game_data: str) -> tuple[set[int], set[int]]:
    ...
    return parse_raw_numbers(winning_numbers_raw), parse_raw_numbers(
        my_numbers_raw
    )
...
def do_part_one(filename: str) -> int:
    with open(filename, "r") as file:
        return sum(
            get_value_of_scorecard(line) for line in file.read().splitlines()
        )
...
def do_part_two(filename: str) -> int:
    with open(filename, "r") as file:
        numbers_of_matches = [
            determine_number_of_matches(line)
