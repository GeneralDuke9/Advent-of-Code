...
def is_valid_part_number(match: Match, row_number: int, all_data: str) -> bool:
    ...
    if start_pos != 0 and all_data[row_number][first_match_pos] != ".":
        return True
    if end_pos != len(all_data[row_number]) and all_data[row_number][last_match_pos] != ".":
        return True
    if row_number != 0 and not all(all_data[row_number-1][idx] == "." for idx in range(first_match_pos, last_match_pos+1)):
        return True
    if row_number != len(all_data)-1 and not all(all_data[row_number+1][idx] == "." for idx in range(first_match_pos, last_match_pos+1)):
        return True
    return False
    

def get_part_number_if_valid(match: Match, row_number: int, all_data: str) -> int:
    if is_valid_part_number(match, row_number, all_data):
        return int(match.group())
    return 0

def get_gear_ratio_if_valid(match: Match, row_number: int, all_data: str) -> int:
    if len(surrounding_numbers := get_surrounding_numbers(match, row_number, all_data)) == 2:
        # print("Valid match")
        # print(surrounding_numbers)
        return prod(surrounding_numbers)
    return 0

def get_surrounding_numbers(match: Match, row_number: int, all_data: str) -> tuple[int, ...]:
    start_pos = match.start()
    end_pos = match.end()
    surrounding_numbers = []
    # breakpoint()
    if row_number != 0:
        numbers_in_row_above = NUMBER_PATTERN.finditer(all_data[row_number-1])
        for match_ in numbers_in_row_above:
            if set(range(match_.start(), match_.end()+1)).intersection(range(start_pos, end_pos + 1)) != set():
    ...
    if row_number != len(all_data) - 1:
        numbers_in_row_below = NUMBER_PATTERN.finditer(all_data[row_number+1])
